...
const crawlProblems = async () => {
  ...
  for (const problem of orgProblems) {
    ...
    if (stat["total_submitted"]) {
      acceptance =
        ((stat["total_acs"] / stat["total_submitted"]) * 100).toFixed(2) + "%";
    }
    ...
    for (const p of Object.values(problems)) {
      ...
      try {
        ...
        const $likes = await page.$(
          "#app > div > div > div > div > div > div > div > div > div > div > div > div > div > button:nth-child(2) > span"
        );
        const $dislikes = await page.$(
          "#app > div > div > div > div > div > div > div > div > div > div > div > div > div > button:nth-child(3) > span"
        );
        const likes = parseInt(
          await page.evaluate((el) => el.textContent, $likes)
        );
        const dislikes = parseInt(
          await page.evaluate((el) => el.textContent, $dislikes)
        );
        problems[p.problemNumber].likes = likes;
        problems[p.problemNumber].dislikes = dislikes;
        if (likes) {
          problems[p.problemNumber].rate =
